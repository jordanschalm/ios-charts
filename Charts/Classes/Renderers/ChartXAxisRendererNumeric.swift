...
public class ChartXAxisRendererNumeric: ChartXAxisRenderer
{
    /// the minimum value on the x-axis
    public var chartMinX: Double = 0.0
    
    /// the maximum value on the x-axis
    public var chartMaxX: Double = 0.0
    ...
    public override func calcXBounds(chart chart: BarLineChartViewBase, xAxisModulus: Int)
    {
        ...
        minX = Int(ceil(low))
        maxX = Int(ceil(high))
        chartMaxX = chart.chartXMax
        chartMinX = chart.chartXMin
    ...
    public override func drawLabels(context context: CGContext, pos: CGFloat, anchor: CGPoint)
    {
        ...
        for (var i = 0; i < xAxis.values.count; i++)
        {
            ...
            {
                let labelns = String(Double(xAxis.values[i]!)!)
